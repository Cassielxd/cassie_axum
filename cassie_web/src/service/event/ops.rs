...
use crate::{APPLICATION_CONTEXT, service::{sys_dict_service::get_all_list, sys_user_service::SysUserService, crud_service::CrudService}};
...
fn op_user_info(id:String) -> Result<SysUserDTO, deno_core::error::AnyError> {
    ...
    let vo = async_std::task::block_on(async {
        user_service.get(id).await }); 
    Ok(vo.unwrap())
}
