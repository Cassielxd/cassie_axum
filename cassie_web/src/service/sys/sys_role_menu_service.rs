...
use cassie_domain::{
  dto::sys_role_dto::SysRoleMenuDTO, entity::sys_entitys::SysRoleMenu,
...
impl SysRoleMenuService {
  ...
  pub async fn save_or_update(
    ...
    let r_list = get_menu_list_by_ids(
      &mut rb.as_executor(),
      &menu_id_list.clone().unwrap(),
    )
    .await
    .unwrap();
  ...
  pub async fn delete_by_role_id(&self, role_id: i64) {
    let request_model = get_local().unwrap();
    self
      .del_by_column(SysRoleMenu::role_id(), &role_id.to_string())
      .await;
    let cached_enforcer =
      APPLICATION_CONTEXT.get::<CasbinService>().enforcer.clone();
    let mut lock = cached_enforcer.write().await;
  ...
  pub async fn delete_by_menu_id(&self, menu_id: i64) {
    self
      .del_by_column(SysRoleMenu::menu_id(), &menu_id.to_string())
      .await;
  }
}

impl CrudService<SysRoleMenu, SysRoleMenuDTO, SysRoleQuery>
  for SysRoleMenuService
{
