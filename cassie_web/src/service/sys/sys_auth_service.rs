...
impl SysAuthService {
  ...
  pub async fn sign_in(&self, arg: &SignInDTO) -> Result<SignInVO> {
    ...
    let user: Option<SysUser> = rb
      .fetch_by_wrapper(
        rb.new_wrapper().eq(SysUser::username(), &arg.username()),
      )
      .await?;
  ...
  pub async fn get_user_info_by_token(
    ...
    let user: Option<SysUser> = rb
      ...
      .await?;
    let user = user.ok_or_else(|| {
      Error::from(format!("账号:{} 不存在!", token.username()))
    })?;
  ...
  pub async fn get_user_info(&self, user: &SysUser) -> Result<SignInVO> {
    ...
    sign_vo
      .set_access_token(jwt_token.create_token(cassie_config.jwt_secret())?);
    return Ok(sign_vo);
  }
}
