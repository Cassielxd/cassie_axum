...
use crate::{sdk::{constant::WECHAT_OPEN_URI, client::Client}, json_decode};

use super::{WxMpUserInfo, WxMpAccessToken};
...
pub async fn get_user_info(access_token: &str,openid:&str) -> Result<WxMpUserInfo> {
    let url = format!(
        ...
        API_DOMAIN, access_token,openid
    );
    let api = Client::new();
    let res = api.get(&url).await?;
    let data =json_decode(&res)?;
