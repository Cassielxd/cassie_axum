use cassie_wx::{sdk::aes_crypt::AesCrypt, wxapp::{resolve_data, auth::get_session_key, WxappSessionKey}};

#[tokio::main]
async fn main() {
    
    let data = get_session_key(
         "wx726d55ff001bf506",
         "407659215044413ef53fcf67b0690ac0",
         "043DBFkl2bmD394DPGkl2i97bx3DBFko",
     )
     .await.unwrap();
    let d:WxappSessionKey = serde_json::from_value(data).unwrap();
    
     println!("aaa:{:?}",d);

    
     let data ="";
    
    let d= resolve_data("TSaaBQMoZp7jg2+cYpiUDw==".to_string(),"fMNK7JnhIWKn3o2Akog04Q==".to_string(),data.to_string());
    println!("{:?}", d);
}
