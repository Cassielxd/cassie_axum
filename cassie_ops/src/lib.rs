...
fn core_import() -> TokenStream2 {
    let found_crate =
        crate_name("cassie_engine").expect("cassie_engine not present in `Cargo.toml`");

    match found_crate {
        FoundCrate::Itself => {
            // TODO(@littledivy): This won't work for `cassie_engine` examples
            // since `crate` does not refer to `cassie_engine`.
            // examples must re-export cassie_engine to make this work
...
impl syn::parse::Parse for MacroArgs {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        let vars = syn::punctuated::Punctuated::<Ident, syn::Token![,]>::parse_terminated(input)?;
...
pub fn op(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    quote! {
      ...
      #[doc="Auto-generated by `cassie_ops`, i.e: `#[cassie_op]`"]
      #[doc=""]
      #[doc=#docline]
      #[doc="you can include in a `cassie_engine::Extension`."]
      pub struct #name;
...
fn codegen_args(
    ...
    let decls: TokenStream2 = inputs
        .clone()
        .map(|(i, arg)| codegen_arg(core, arg, format!("arg_{i}").as_ref(), v8_i0 + i))
        .collect();
    (decls, ident_seq)
}

fn codegen_arg(core: &TokenStream2, arg: &syn::FnArg, name: &str, idx: usize) -> TokenStream2 {
...
fn codegen_sync_ret(core: &TokenStream2, output: &syn::ReturnType) -> TokenStream2 {
...
fn is_mut_ref_opstate(arg: &syn::FnArg) -> bool {
    tokens(arg).ends_with(": & mut OpState")
        || tokens(arg).ends_with(": & mut cassie_engine :: OpState")
}

fn is_rc_refcell_opstate(arg: &syn::FnArg) -> bool {
    tokens(arg).ends_with(": Rc < RefCell < OpState > >")
        || tokens(arg).ends_with(": Rc < RefCell < cassie_engine :: OpState > >")
}
