    ...
    impl<T> AsyncRefCell<T> {
        ...
        pub fn borrow_sync<M: BorrowModeTrait, R: RcLike<AsyncRefCell<T>>>(
            ...
            if waiters.is_empty() {
                ...
                let new_borrow_count = cell_ref.borrow_count.get().try_add(M::borrow_mode())?;
        ...
        fn poll_waiter<M: BorrowModeTrait>(&self, id: usize, cx: &mut Context) -> Poll<()> {
    ...
    impl BorrowCount {
        ...
        pub fn try_add(self, mode: BorrowMode) -> Option<BorrowCount> {
            match (self, mode) {
                ...
                (BorrowCount::Shared(0), BorrowMode::Exclusive) => Some(BorrowCount::Exclusive),
                _ => None,
            }
        }
        ...
        pub fn try_remove(self, mode: BorrowMode) -> Option<BorrowCount> {
            match (self, mode) {
                ...
                (BorrowCount::Exclusive, BorrowMode::Exclusive) => Some(BorrowCount::Shared(0)),
                _ => None,
            }
        }
