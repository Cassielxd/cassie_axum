...
((window) => {
  ...
  const ops = window.Cassie.core.ops;

  // Available on start due to bindings.
  const { refOp_, unrefOp_ } = window.Cassie.core;
  ...
  const promiseIdSymbol = SymbolFor("Cassie.core.internalPromiseId");
  ...
  function unwrapOpResult(res) {
    ...
    if (res?.$err_class_name) {
      ...
      const err = errorBuilder ? errorBuilder(res.message) : new Error(
        `Unregistered error class: "${className}"\n  ${res.message}\n  Classes of errors returned from ops should be registered via Cassie.core.registerErrorClass().`,
      );
  ...
  // Extra Cassie.core.* exports
  const core = ObjectAssign(globalThis.Cassie.core, {
    opAsync,
    opSync,
    close,
    tryClose,
  ...
  ObjectAssign(globalThis.Cassie, { core });
})(globalThis);
