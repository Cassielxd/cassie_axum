...
use crate::{include_js_files, OpState, Resource};
use anyhow::Error;
use cassie_ops::op;
...
#[op]
pub fn op_resources(state: &mut OpState) -> Result<Vec<(ResourceId, String)>, Error> {
...
#[op]
pub fn op_close(state: &mut OpState, rid: Option<ResourceId>) -> Result<(), Error> {
...
#[op]
pub fn op_metrics(state: &mut OpState) -> Result<(OpMetrics, Vec<OpMetrics>), Error> {
...
pub fn op_wasm_streaming_feed(
    ...
    let wasm_streaming = state.resource_table.get::<WasmStreamingResource>(rid)?;
...
#[op]
async fn op_shutdown(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<(), Error> {
    let resource = state.borrow().resource_table.get_any(rid)?;
