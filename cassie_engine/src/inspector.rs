...
/// to the inspector, in case of Cassie it's either: a "websocket session" that
...
impl JsRuntimeInspector {
    /// Currently Cassie supports only a single context in `JsRuntime`
    ...
    pub fn create_local_session(&self) -> LocalInspectorSession {
        ...
        let inspector_session = InspectorSession::new(self.v8_inspector.clone(), proxy);
        self.sessions
            .borrow_mut()
            .established
...
impl InspectorSession {
    ...
    fn dispatch_message(v8_session_ptr: *mut v8::inspector::V8InspectorSession, msg: String) {
...
impl LocalInspectorSession {
    ...
    pub async fn post_message<T: serde::Serialize>(
        ...
        let (response_tx, mut response_rx) = oneshot::channel::<serde_json::Value>();
    ...
    async fn receive_from_v8_session(&mut self) {
        ...
        if let InspectorMsgKind::Message(msg_id) = inspector_msg.kind {
            let message: serde_json::Value = match serde_json::from_str(&inspector_msg.content) {
                Ok(v) => v,
